...
function InteractiveLessonCard({
  ...
  loading: boolean;
  error: string | null;
  onGenerate: (topicId: string) => Promise<InteractiveLesson | null>;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[#64748b]">
            由 OpenMAIC 外部服务生成互动式课堂。生成过程不会修改你的学习状态，也不会把学号、代码或私有记录发送出去。
          </p>
        </div>
        {lesson ? (
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {!lesson || lesson.status === "failed" ? (
          <button
            ...
            disabled={loading}
            onClick={() => void onGenerate(topicId)}
            type="button"
          >
